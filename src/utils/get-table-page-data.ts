...
export const getTableData = <T extends any[]>(pageNum = 1, pageSize = 10, totalData: T) => {
  ...
  if (pageSize >= total) {
    ...
    tableData.records = totalData;
    tableData.current = 1; //直接取第一页
  } else {
    ...
    if (num < total) {
      ...
      tableData.records = totalData.filter((_, index) => index >= startIndex && index <= endIndex); //当前页数据条数小于每页最大条数时，也按最大条数范围筛取数据
    } else {
      ...
      if (rest > 0) {
        ...
        tableData.current = size + 1; //当前页码重置，取size+1
        tableData.records = totalData.filter((_, index) => index >= pageSize * size && index <= total);
      } else if (rest === 0) {
        //余数等于0，最后一页数据条数正好是pageSize
        tableData.current = size; //当前页码重置，取size
        tableData.records = totalData.filter((_, index) => index >= pageSize * (size - 1) && index <= total);
      } //注：余数不可能小于0
    }
  }
  return tableData;
